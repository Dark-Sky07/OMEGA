...
export function buildAmneziaWGClientConfig(
  ...
  const address = addressOverride || clientAllowedIPs(client) || '10.8.1.2/32';
  ...
  if (!privateKey || !server?.publicKey || !endpointHost || !inbound?.port) return '';
...
export function buildAmneziaWGClientLink(
  ...
  const config = buildAmneziaWGClientConfig(client, inbound, host, publicHost, addressOverride);
  if (!config) return '';
