...
export const sections: readonly Section[] = [
...

  {
    ...
    description:
      'Panel sub-accounts that own a slice of the panel: their own inbounds, their own clients, and two quotas (clients, traffic). Inbounds are attached by the admin via the Attach Inbound list — the reseller can only see/manage assigned inbounds. A reseller signs in with its own username/password on the same login page; its session reads <code>/panel/api/inbounds/*</code> and <code>/panel/api/reseller/*</code>, manages its clients under <code>/panel/api/clients/*</code> and reaches <code>/panel/api/auth/me</code> — inbound writes and everything else answer 403. The endpoints below are the admin-side management API under <code>/panel/api/resellers</code>; a wrong-owner access answers 403 with <code>inbound not found</code> / <code>client not found</code>.',
    endpoints: [
      {
        method: 'GET',
        path: '/panel/api/resellers/list',
        summary:
          'Every reseller with its live usage snapshot: inbound/client counts, allocated and consumed traffic and quota flags.',
        response:
          '{\n  "success": true,\n  "obj": [\n    {\n      "reseller": { "id": 1, "username": "reseller1", "trafficLimit": 1099511627776, "clientLimit": 100, "clientLimit": 100 },\n      "inboundCount": 2,\n      "clientCount": 12,\n      "usedTraffic": 10737418240,\n      "allocatedTraffic": 549755813888\n    }\n  ]\n}',
      },
      {
        method: 'GET',
        path: '/panel/api/resellers/get/:id',
        summary: 'One reseller with the same usage snapshot as the list endpoint.',
      ...
      {
        method: 'GET',
        path: '/panel/api/resellers/report/:id',
        summary:
          'Full usage report of one reseller: usage snapshot and one row per client (quota, usage, attached inbounds).',
        params: [{ name: 'id', in: 'path', type: 'integer', desc: 'Reseller id.' }],
      },
      {
        method: 'POST',
        ...
        summary:
          'Create a reseller. The password is hashed with bcrypt; a zero quota means "no limit". The username must differ from the panel admin and from every other reseller.',
        params: [
          { name: 'username', in: 'body (json)', type: 'string' },
          { name: 'password', in: 'body (json)', type: 'string' },
          { name: 'name', in: 'body (json)', type: 'string', optional: true },
          { name: 'comment', in: 'body (json)', type: 'string', optional: true },
      ...
      },
      {
        ...
        summary: 'Update a reseller. Send an empty (or omitted) password to keep the stored one.',
        params: [{ name: 'id', in: 'path', type: 'integer', desc: 'Reseller id.' }],
      },
      {
        method: 'POST',
        ...
        summary:
          'Delete a reseller. Its inbounds and clients stay on the panel and fall back to the admin; ownership mappings and the ledger are removed.',
        params: [{ name: 'id', in: 'path', type: 'integer', desc: 'Reseller id.' }],
      },
      {
        method: 'POST',
        path: '/panel/api/resellers/setEnable/:id',
        summary:
          'Enable or disable a reseller. Disabling also turns off every inbound it owns and invalidates its active sessions.',
        params: [
          { name: 'id', in: 'path', type: 'integer', desc: 'Reseller id.' },
          { name: 'enable', in: 'body (json)', type: 'boolean' },
        ],
      },
      {
        method: 'POST',
        path: '/panel/api/resellers/resetPassword/:id',
        summary: 'Set a new password for a reseller and invalidate its sessions.',
        params: [
          { name: 'id', in: 'path', type: 'integer', desc: 'Reseller id.' },
          { name: 'password', in: 'body (json)', type: 'string' },
        ],
      ...
      {
        method: 'POST',
        path: '/panel/api/resellers/assignInbound',
        summary: 'Hand an inbound (with all of its clients) to a reseller.',
        params: [
      ...
      {
        method: 'POST',
        path: '/panel/api/resellers/unassignInbound',
        summary:
          'Return an inbound to the admin. Clients that only belonged to the reseller through this inbound go back with it; explicitly assigned clients stay.',
        params: [
          { name: 'resellerId', in: 'body (json)', type: 'integer' },
          { name: 'inboundId', in: 'body (json)', type: 'integer' },
        ],
      ...
      {
        method: 'POST',
        path: '/panel/api/resellers/assignClient',
        summary:
          'Assign a single client (by email) to a reseller. Used for clients that live on a shared inbound owned by the admin.',
        params: [
          { name: 'resellerId', in: 'body (json)', type: 'integer' },
          { name: 'email', in: 'body (json)', type: 'string' },
        ],
      },
      {
        method: 'POST',
        path: '/panel/api/resellers/unassignClient',
        summary: 'Remove an explicit client assignment.',
        params: [
          { name: 'resellerId', in: 'body (json)', type: 'integer' },
          { name: 'email', in: 'body (json)', type: 'string' },
        ],
      ...
      {
        method: 'POST',
        path: '/panel/api/resellers/balance',
        summary:
          'Top up (positive amount) or settle (negative amount) a reseller balance and append the movement to its ledger. A withdrawal cannot push the balance below zero.',
        params: [
          { name: 'resellerId', in: 'body (json)', type: 'integer' },
          { name: 'amount', in: 'body (json)', type: 'number' },
          { name: 'comment', in: 'body (json)', type: 'string', optional: true },
        ],
      },
      {
        method: 'GET',
        path: '/panel/api/reseller/profile',
        summary: 'The signed-in reseller own profile and live usage snapshot.',
      },
      {
        method: 'GET',
        path: '/panel/api/reseller/stats',
        summary: 'Same payload as /panel/api/reseller/profile — alias used by the reseller dashboard.',
      },
      {
        method: 'GET',
        path: '/panel/api/reseller/report',
        summary: 'The signed-in reseller own usage/billing report (clients + ledger).',
      },
      {
        method: 'POST',
        path: '/panel/api/reseller/password',
        summary:
          'Change the signed-in reseller own password. The old password is verified first; the session is cleared afterwards because the login epoch is bumped.',
        params: [
          { name: 'oldPassword', in: 'body (json)', type: 'string' },
          { name: 'newPassword', in: 'body (json)', type: 'string' },
        ],
      },
      {
        method: 'GET',
        path: '/panel/api/auth/me',
        summary:
          'Identity of the caller, so the SPA can render the right navigation: <code>role</code> is <code>admin</code> or <code>reseller</code>.',
        response:
          '{\n  "success": true,\n  "obj": { "role": "reseller", "username": "reseller1", "name": "Ali", "stat": { "clientCount": 12 } }\n}',
      },
    ],
  },
