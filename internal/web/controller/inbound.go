...
func (a *InboundController) getInboundsSlim(c *gin.Context) {
	if reseller := resellerSession(c); reseller != nil {
		inbounds, err := a.inboundService.GetInboundsSlimForReseller(reseller.Id)
		if err != nil {
			jsonMsg(c, I18nWeb(c, "pages.inbounds.toasts.obtain"), err)
			return
		}
...
func (a *InboundController) getInboundOptions(c *gin.Context) {
	if reseller := resellerSession(c); reseller != nil {
		options, err := a.inboundService.GetInboundOptionsForReseller(reseller.Id)
		if err != nil {
			jsonMsg(c, I18nWeb(c, "pages.inbounds.toasts.obtain"), err)
			return
		}
