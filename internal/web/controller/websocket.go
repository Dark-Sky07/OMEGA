...
func (w *WebSocketController) HandleWebSocket(c *gin.Context) {
	...
	// The hub broadcasts panel-wide payloads (inbound lists, server status), so
	// reseller sessions are kept off the socket entirely: they get their data
	// through the REST endpoints, which are scoped to their own objects.
