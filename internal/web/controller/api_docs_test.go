...
func TestAPIRoutesDocumented(t *testing.T) {
	...
	for _, entry := range entries {
		...
		case "reseller.go":
			basePath = "/panel/api/resellers"
		case "reseller_self.go":
			basePath = "/panel/api/reseller"
		case "auth.go":
			basePath = "/panel/api/auth"
		case "websocket.go":
			basePath = ""
		}
	...
	for _, r := range allRoutes {
		...
		spaPages := map[string]bool{
			...
			"/panel/resellers": true, "/panel/reseller/report": true,
			"/panel/reseller/profile": true,
		}
		if spaPages[r.Path] {
			continue
		}
