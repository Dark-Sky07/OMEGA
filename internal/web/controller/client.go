...
func (a *ClientController) get(c *gin.Context) {
	...
	// A shared client identity can have different tunnel addresses on
	// different WireGuard-family inbounds. Keep that per-inbound projection
	// separate from ClientRecord.AllowedIPs, whose legacy column can only hold
	// one comma-separated value. Scope the lookup to the same inbound IDs the
	// caller is allowed to see (especially for resellers); a malformed legacy
	// settings blob remains non-fatal, like the traffic projection above.
	allowedIPsByInbound, _ := a.clientService.TunnelAllowedIPsByInbound(&a.inboundService, email, inboundIds)
	jsonObj(c, gin.H{
		"client":              rec,
		"inboundIds":          inboundIds,
		"usedTraffic":         usedTraffic,
		"allowedIPsByInbound": allowedIPsByInbound,
	}, nil)
}
