...
func (a *ClientController) list(c *gin.Context) {
	...
	if reseller := resellerSession(c); reseller != nil {
		...
		isolated, err := newResellerService().OwnedIsolatedAssociatedEmailSet(reseller.Id)
		if err != nil {
			jsonMsg(c, I18nWeb(c, "pages.inbounds.toasts.obtain"), err)
			return
		}
		...
		for _, row := range rows {
			...
			if _, isIsolated := isolated[strings.ToLower(strings.TrimSpace(row.Email))]; !isIsolated || len(row.InboundIds) == 0 {
				// An explicit mapping without a reseller-owned
				// attachment, or a client shared with an admin-owned
				// inbound, cannot expose the email-level aggregate.
				row.Traffic = nil
			} else if row.Traffic != nil {
				traffic := *row.Traffic
				traffic.InboundId = 0
				row.Traffic = &traffic
			}
