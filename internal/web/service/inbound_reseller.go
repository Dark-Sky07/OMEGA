...
// not explicitly mapped to the reseller. It is applied after enrichment and
// traffic overlays so neither settings.clients nor clientStats can disclose a
// client merely because it shares a reseller-owned inbound.
func filterResellerInboundClients(inbounds []*model.Inbound, allowed map[string]struct{}) {
	for _, inbound := range inbounds {
		inbound.ClientStats = filterResellerClientStats(inbound.ClientStats, allowed)
		inbound.Settings = filterResellerSettingsClients(inbound.Settings, allowed)
	}
}
...
func (s *InboundService) GetInboundsForReseller(resellerId int) ([]*model.Inbound, error) {
	...
	filterResellerInboundClients(inbounds, allowed)
	return inbounds, nil
}
...
func (s *InboundService) GetInboundDetailForReseller(resellerId, inboundId int) (*model.Inbound, error) {
	...
	s.annotateFallbackParents(db, []*model.Inbound{inbound})
	s.annotateLocalOriginGuid([]*model.Inbound{inbound})
	filterResellerInboundClients([]*model.Inbound{inbound}, allowed)
	return inbound, nil
}
