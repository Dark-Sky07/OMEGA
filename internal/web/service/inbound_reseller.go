...
// not explicitly mapped to the reseller. ClientTraffic is an email-level
// aggregate, so scoped responses retain counters only after the explicit
// mapping check and redact the legacy inbound identifier.
...
func (s *InboundService) GetInboundsForReseller(resellerId int) ([]*model.Inbound, error) {
	...
	// ClientStats visibility follows the explicit ResellerClient mapping. An
	// aggregate traffic row may include bytes from another inbound, but the
	// mapping is the tenant authorization boundary and inboundId is scrubbed
	// from scoped traffic responses.
