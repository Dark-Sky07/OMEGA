...
func TestOwnedAssociatedEmailSetRequiresExplicitMappingAndOwnedAssociation(t *testing.T) {
	...
	if err := db.Create(&xray.ClientTraffic{
		...
		t.Fatalf("create shared traffic row: %v", err)
	}
	...
	if traffic == nil || traffic.Up != 321 || traffic.Down != 654 {
		t.Fatalf("explicitly mapped shared client traffic was hidden: %+v", traffic)
	}
	if traffic.InboundId != 0 {
		t.Fatalf("scoped shared traffic leaked unrelated inbound id %d", traffic.InboundId)
	}
