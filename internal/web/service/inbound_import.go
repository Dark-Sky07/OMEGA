...
func (s *InboundService) PrepareInboundImport(inbound *model.Inbound) error {
	...
	for index, raw := range rawClients {
		...
		imported.Email = strings.TrimSpace(imported.Email)
		if imported.Email == "" {
			continue
		}
		if clientMap, ok := raw.(map[string]any); ok {
			clientMap["email"] = imported.Email
			rawClients[index] = clientMap
		}
		...
		if err != nil && (database.IsNotFound(err) || err == gorm.ErrRecordNotFound) {
			...
			lookupErr := database.GetDB().Where("LOWER(TRIM(email)) = LOWER(?)", strings.TrimSpace(imported.Email)).First(candidate).Error
			if lookupErr == nil {
				existing = candidate
				err = nil
