...
type ClientPageParams struct {
	...
	// ScopeEmails, when non-nil, restricts the result to an explicit set of
...
func (s *ClientService) ListPaged(inboundSvc *InboundService, settingSvc *SettingService, params ClientPageParams) (*ClientPageResponse, error) {
	...
	if params.ScopeEmails != nil {
		// Reseller session: everything below (totals, summary, groups) must be
	...
	if params.ScopeEmails != nil {
		// The explicit ResellerClient mapping authorizes aggregate online and
		// traffic state. It is not safe to infer visibility from inbound
		// ownership because a mapped client may also be attached to an admin
		// inbound; the aggregate's inboundId is redacted below.
		for i := range all {
