...
func (s *InboundService) MigrationRemoveOrphanedTraffics() error {
	db := database.GetDB()
	query := fmt.Sprintf(
		"DELETE FROM client_traffics WHERE NOT EXISTS (SELECT 1 %s WHERE LOWER(TRIM(%s)) = LOWER(TRIM(client_traffics.email)))",
...
func (s *InboundService) MigrationRequirements() (err error) {
	...
	if database.IsPostgres() {
		...
		for _, column := range [][2]string{
			...
			{"clients", "enable"},
			{"api_tokens", "enabled"},
			{"outbound_subscriptions", "enabled"},
		} {
	...
	for inbound_index := range inbounds {
		...
		modelClients, clientErr := s.GetClients(inbounds[inbound_index])
		if clientErr != nil {
			err = clientErr
			return err
		}
		for _, modelClient := range modelClients {
			if len(modelClient.Email) > 0 {
				var count int64
				if countErr := tx.Model(xray.ClientTraffic{}).
					Where("LOWER(TRIM(email)) = LOWER(?)", strings.TrimSpace(modelClient.Email)).
					Count(&count).Error; countErr != nil {
					err = countErr
					return err
				}
				if count == 0 {
					if statErr := s.AddClientStat(tx, inbounds[inbound_index].Id, &modelClient); statErr != nil {
						err = statErr
						return err
					}
				}
	...
	if deleteErr := tx.Where("inbound_id = 0").Delete(xray.ClientTraffic{}).Error; deleteErr != nil {
		err = deleteErr
		return err
	}
	...
	for _, ep := range externalProxy {
		...
		if updateErr := tx.Model(model.Inbound{}).Where("id = ?", ep.Id).
			Update("stream_settings", newStream).Error; updateErr != nil {
			err = updateErr
			return err
		}
	}
...
func (s *InboundService) MigrateDB() error {
	...
	return s.MigrationRemoveOrphanedTraffics()
}
