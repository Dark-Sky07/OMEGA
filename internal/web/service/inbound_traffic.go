...
// GetClientTrafficByEmailForInbounds first proves that every current client
// association is inside the allowed inbound set, then returns the normal
// email-aggregate traffic row. ClientTraffic is one shared row per canonical
// client, so returning it for a client shared with an out-of-scope inbound
// would disclose usage that cannot be partitioned by tenant.
func (s *InboundService) GetClientTrafficByEmailForInbounds(email string, allowedInboundIDs map[int]struct{}) (*xray.ClientTraffic, error) {
	...
	// an administrator or another reseller even after the client has passed the
	// complete-association scope check above. Never return that stale routing
