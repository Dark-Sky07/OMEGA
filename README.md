...
```bash
bash <(curl -Ls https://raw.githubusercontent.com/Dark-Sky07/OMEGA/v3.3.20-omega/install-omega.sh) v3.3.20-omega
```
...
> version is maintained separately from the upstream core version, so the current stable release is `v3.3.20-omega`.
...
```bash
curl -fsSL https://raw.githubusercontent.com/Dark-Sky07/OMEGA/v3.3.20-omega/install-omega.sh \
  -o /tmp/install-omega.sh
env OMEGA_REF=v3.3.20-omega bash /tmp/install-omega.sh v3.3.20-omega
```
